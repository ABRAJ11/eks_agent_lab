...
resource "helm_release" "milvus" {
  ...
  values = [yamlencode({
    ...
    minio = {
      enabled = true
      mode    = "standalone"
      # MinIO deleted the docker.io/minio/minio repository outright: the Docker
      # Hub repo itself 404s and its tags/list is empty, so the chart default
      # (minio/minio:RELEASE.2024-12-18T13-15-44Z) is unpullable and there is no
      # newer Docker Hub tag to move to. quay.io is MinIO's remaining public
      # registry and still serves this exact tag anonymously with linux/amd64.
      # The repository is pinned, not just the tag, because the subchart renders
      # "{{ .Values.image.repository }}:{{ .Values.image.tag }}" with no separate
      # registry key, so the host has to live in repository.
      image = {
